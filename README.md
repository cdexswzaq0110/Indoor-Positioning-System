...
### 量化 domain gap：合成地板測試集

公開資料集裡**沒有**「地板數字」這種東西——找過 Roboflow Universe、Kaggle、
Chars74K、ICDAR 與相關論文，有的是建築平面圖、停車格佔用、街景門牌。
用 floor camera 做定位的論文（[arXiv 2504.03249](https://arxiv.org/pdf/2504.03249)）
自己就寫了他們在實驗室錄資料，理由是 lack of available datasets。

所以 `ml/make_floor_testset.py` 合成一組**幾何對得上**的測試集：手機在約 1–1.5 m 高、
俯角 30–65 度看地板，含透視前縮、磁磚材質、光照漸層、手震模糊與 JPEG 劣化。

```bash
.venv\Scripts\python.exe ml\make_floor_testset.py --n 400
.venv\Scripts\python.exe -m ultralytics val model=ml/weights/best.pt data=ml/data/floor_test/floor_test.yaml imgsz=320
```

SVHN 訓練的模型在這組上的結果（400 張，數字 1–4）：

| | SVHN val | 地板測試集 |
|---|---|---|
| mAP50 | 0.905 | 0.854 |
| mAP50-95 | 0.433 | **0.478** |
| Precision | 0.903 | 0.943 |
| Recall | 0.867 | 0.785 |

定位精度（mAP50-95）反而更好——地板地標又大又乾淨，比遠距離的小門牌好定位。

### ⚠ 不要用數字 1 當地標

逐類別拆開來看，總體數字蓋掉了一個功能性缺陷：
...
**「1」有一半偵測不到。** 它是一根細直筆畫，在地板透視前縮下變得更細，
而 SVHN 的「1」幾乎都是正面平視的門牌。使用者站在 1 號區，有一半機率系統當作沒看到——
這是功能性缺陷，不是精度問題。**實際場域請用 2–9 當地標，或給 1 加上可區辨的外框。**

> 這組是**合成**資料，字型乾淨、沒有磨損污漬，比真實地板容易。
> 0.854 是樂觀上限，不是真實場域的預測值。它的用途是比較與抓出像「1」這種結構性問題，
> 不是拿來對外宣稱準確率。
